...
ChemList::ChemList()
{
	...
	readReactions();
    for(int i=0;i<numReact;i++) // Temperature array for reaction coefficient
    {
        reactTemCoef[i] = 1;
        kobs_React[i] = kobs_React[i]*reactTemCoef[i];	
        printf("reaction type=%d, reactionCef=%f\n",i,reactTemCoef[i]);
    }
	setReactantList();
	setReactRadius();
}
...
void ChemList::readRadiolyticSpecies()
{
	...
	for (int i = 0; i < numSpecType; i++)
	{
	   ...
	   diffCoef_spec[i] = diffCoef_spec[i] * 1.0e-3f; // convert the diffusion coefficient from 10^9 nm^2/s to nm^2/ps
...
{// for number of radicals vs time
	...
	unsigned char*h_index=(unsigned char*) malloc(sizeof(unsigned char) * numCurPar*2);
	...
	cudaMemcpy(h_index,d_index,sizeof(unsigned char)*numCurPar, cudaMemcpyDeviceToHost);
	
	// Open pTuple file
	float pTuple[6];
	std::string fname = document["pTupleFile"].GetString();
	FILE* fptuple=fopen(fname.c_str(),"ab");

	int numOfSpec[numSpecType];
	for(int tmptmp =0;tmptmp<numSpecType;tmptmp++)
		numOfSpec[tmptmp] = 0;
	for(int tmptmp=0;tmptmp<numCurPar;tmptmp++)
	{	
		if(h_ptype[tmptmp]>numSpecType) printf("\n\nSomething is wrong...\n\n");
		
		// Write pTuple
		pTuple[0] = h_posx[tmptmp];
		pTuple[1] = h_posy[tmptmp];
		pTuple[2] = h_posz[tmptmp];
		pTuple[3] = curTime;
		pTuple[4] = h_index[tmptmp];
		pTuple[5] = h_ptype[tmptmp];
		fwrite(&pTuple[0],sizeof(float),6,fptuple);
